                ...
                <div class="card-body">
                    ...
                    <form method="post" action="{{ route('admin.course.update',$course) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-x-5 md:grid-cols-2 xl:grid-cols-3">
                            ...
                            <x-input-label : id="course_name" value="{{ old('course_name',$course->course_name) }}" title="Course Name"
                                spanClass="text-red-500" type="text" />
                            <x-input-label : id="fee" value="{{ old('fee',$course->fee) }}" title="Course Fee"
                                spanClass="text-red-500" type="number" />
                  
                            <x-input-label : id="duration" value="{{ old('duration',$course->duration) }}" spanClass="text-red-500"
                                title="Duration" type="text" />
                            <x-input-label : id="start_date" value="{{ old('start_date',$course->start_date) }}"
                                spanClass="text-red-500" title="Start Date" type="date" />
                            <x-input-label : id="end_date" value="{{ old('end_date',$course->end_date) }}"
                                spanClass="text-red-500" title="End Date" type="date" />
                                <x-select id="course_type" value="{{ old('course_type',$course->course_type) }}" title="Course Type" spanClass="text-white"
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea name="required_documents" id="editor" cols="50" rows="10">{{ old('required_documents',$course->required_documents) }}</textarea>
